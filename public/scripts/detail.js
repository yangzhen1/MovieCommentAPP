...
window.onload = () => {
  ...
  db.collection("movies").doc(id).get().then(doc => {
    ...
    let result = `
      ...
      <div class="button" ${email ? '' : 'style="display: none"'}>
        <a href="form.html?id=${id}&flag=0">Want to</a>
        <a href="form.html?id=${id}&flag=1">Watched</a>
      </div>
      <h3>Description</h3>
      <p>
        ${movie.Plot}
    ...
    `
    result += `<h3>Director</h3><ul>`
    movie.Director.forEach(item => {
      result += `
        <li>${item}</li>
      `
    })
    result += `</ul><h3>Actors</h3><ul>`
    movie.Actors.forEach(item => {
      result += `
        <li>${item}</li>
      `
    })
    result += `</ul><h3>Writers</h3><ul>`
    ...
    (movie['Comment'] || []).forEach(comment => {
      result += `
        <div class="comment" style="margin-bottom: 5px;">
          <div>${comment.email}: ${comment.star}</div>
          <p>${comment.comment}</p>
        </div>
      `
    })
